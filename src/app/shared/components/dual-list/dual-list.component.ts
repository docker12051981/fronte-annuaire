...
export class DualListComponent implements DoCheck, OnChanges {
  ...
  static DEFAULT_FORMAT = { add: 'Company List', remove: 'Supervised structures', all: 'All', none: 'None', direction: DualListComponent.LTR, draggable: true, placeholder: 'Search' };
  static FR_FORMAT = { add: 'Liste des Structures', remove: 'Structures supervisées', all: 'TOUT', none: 'AUCUN', direction: DualListComponent.LTR, draggable: true, placeholder: 'Recherche' };
  static AR_FORMAT = { add: 'قائمة الهياكل', remove: 'هياكل الإشراف', all: 'الكل', none: 'لا شيئ', direction: DualListComponent.RTL, draggable: true, placeholder: 'بحث' };
  ...
  @Input() format_fr = DualListComponent.FR_FORMAT;
  @Input() format_ar = DualListComponent.AR_FORMAT;
  @Input() sort = false;
  ...
  ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
    ...
    if (changeRecord['format_fr']) {
      this.format_fr = changeRecord['format_fr'].currentValue;

      if (typeof (this.format_fr.direction) === 'undefined') {
        this.format_fr.direction = DualListComponent.LTR;
      }

      if (typeof (this.format_fr.add) === 'undefined') {
        this.format_fr.add = DualListComponent.FR_FORMAT.add;
      }

      if (typeof (this.format_fr.remove) === 'undefined') {
        this.format_fr.remove = DualListComponent.FR_FORMAT.remove;
      }

      if (typeof (this.format_fr.all) === 'undefined') {
        this.format_fr.all = DualListComponent.FR_FORMAT.all;
      }

      if (typeof (this.format_fr.none) === 'undefined') {
        this.format_fr.none = DualListComponent.FR_FORMAT.none;
      }

      if (typeof (this.format_fr.draggable) === 'undefined') {
        this.format_fr.draggable = DualListComponent.FR_FORMAT.draggable;
      }
    }

    if (changeRecord['format_ar']) {
      this.format_ar = changeRecord['format_ar'].currentValue;

      if (typeof (this.format_ar.direction) === 'undefined') {
        this.format_ar.direction = DualListComponent.RTL;
      }

      if (typeof (this.format_ar.add) === 'undefined') {
        this.format_ar.add = DualListComponent.AR_FORMAT.add;
      }

      if (typeof (this.format_ar.remove) === 'undefined') {
        this.format_ar.remove = DualListComponent.AR_FORMAT.remove;
      }

      if (typeof (this.format_ar.all) === 'undefined') {
        this.format_ar.all = DualListComponent.AR_FORMAT.all;
      }

      if (typeof (this.format_ar.none) === 'undefined') {
        this.format_ar.none = DualListComponent.AR_FORMAT.none;
      }

      if (typeof (this.format_ar.draggable) === 'undefined') {
        this.format_ar.draggable = DualListComponent.AR_FORMAT.draggable;
      }
    }
  ...
  direction() {
    if (JSON.parse(localStorage.getItem('config')).lang == "en") {
      return this.format.direction === DualListComponent.LTR;
    }
    if (JSON.parse(localStorage.getItem('config')).lang == "fr") {
      return this.format_fr.direction === DualListComponent.LTR;
    }
    if (JSON.parse(localStorage.getItem('config')).lang == "ar") {
      return this.format_ar.direction === DualListComponent.RTL;
    }
  }
